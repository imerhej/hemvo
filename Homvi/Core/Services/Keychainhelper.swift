...
//  All items use kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly by default,
//  which means they survive app deletion and reinstall but stay on-device.
//
//  For cross-device account recovery, use kSecAttrAccessibleAfterFirstUnlock
//  (without ThisDeviceOnly) so iCloud Keychain can sync the item.
//  We use the iCloud-syncable variant for user credentials so that reinstalling
//  the app on the SAME device OR a new device restores the account automatically.
...
final class KeychainHelper {
    ...
    // Service identifier — scopes all Homvi items in the Keychain.
    private let service = "com.homvi.app"
    ...
    func save(_ data: Data, key: String, iCloudSync: Bool = true) -> Bool {
        let accessibility: CFString = iCloudSync
            ? kSecAttrAccessibleAfterFirstUnlock          // syncs via iCloud Keychain
    ...
    /// Delete ALL Homvi Keychain items — call on account deletion only.
    @discardableResult
    func deleteAll() -> Bool {
        let query: [CFString: Any] = [
            kSecClass:       kSecClassGenericPassword,
