...
struct WarmPersonalSection: View {
    ...
    // Dirty check — has the user changed anything?
    var isDirty: Bool {
    ...
    var body: some View {
        VStack(spacing: 18) {
            ...
            VStack(spacing: 0) {
                ...
                infoRow(
                    label: "Account ID",
                    value: String(authVM.profile?.id.uuidString.prefix(8) ?? "—") + "…"
                )
            }
            .background(Color.white)
            .cornerRadius(18)
    ...
    private func save() {
        ...
        Task {
            do {
                ...
                let avatarColor = UserDefaults.standard.string(forKey: "hb_avatarColor") ?? "#C8922A"
            ...
            if newValue {
                Task { @MainActor in
                    ...
                    if !ok {
                        ...
                        biometricError = "\(authVM.biometricLabel) could not be enabled. Check Settings → Privacy → Homvi."
    ...
    var body: some View {
        HStack(spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Group {
                    if showText {
                        TextField(label, text: $text).autocorrectionDisabled().textInputAutocapitalization(.never)
                    } else {
        ...
        VStack(spacing: 16) {
            if cards.isEmpty {
                VStack(spacing: 14) {
                    ...
                    Text("Add a card to manage your Homvi subscription.")
