...
struct SettingsView: View {
    ...
    // ── Profile helpers (from Supabase HomviProfile) ───────────────────────
    private var displayName: String {
        authVM.profile?.fullName ?? "Homvi User"
    }
    private var displayEmail: String {
        authVM.profile?.email ?? ""
    }
    private var initials: String {
        let parts = (authVM.profile?.fullName ?? "")
            .split(separator: " ").prefix(2)
        return parts.map { String($0.prefix(1)).uppercased() }.joined()
            .isEmpty ? "HV" : parts.map { String($0.prefix(1)).uppercased() }.joined()
    }
    ...
    var body: some View {
        ...
        .alert("Sign Out", isPresented: $showSignOutAlert) {
            ...
            Text("Are you sure you want to sign out of Homvi?")
        }
        .alert("Delete Account", isPresented: $showDeleteAlert) {
            Button("Continue", role: .destructive) { showDeleteConfirm = true }
            Button("Cancel", role: .cancel) { }
        } message: {
            Text("This will permanently delete your account and remove all Homvi data from this device — meals, budget, schedule, maintenance, and your login. This cannot be undone.")
        }
        .alert("Are you absolutely sure?", isPresented: $showDeleteConfirm) {
            Button("Delete My Account", role: .destructive) {
                // deleteAccount is async — wrap in Task
        ...
        .alert("Notifications Blocked", isPresented: $showNotifAlert) {
            ...
            Text("Enable notifications in Settings → Homvi → Notifications to receive reminders.")
        }
    }
    ...
    private var heroHeader: some View {
        ZStack(alignment: .bottom) {
            ...
            Circle().fill(Color(hex: "#FFCCBC")!.opacity(0.1)).frame(width: 150).offset(x: -80, y: 60)
                ...
                HStack(alignment: .top, spacing: 16) {
                    ...
                    ZStack {
                        ...
                        Circle()
                            .fill(
                                LinearGradient(
                                    colors: [Color(hex: "#FFCCBC")!, Color(hex: "#FF8A65")!],
                                    startPoint: .topLeading, endPoint: .bottomTrailing
                                )
                            )
                            .frame(width: 66, height: 66)
                            .shadow(color: Color(hex: "#BF360C")!.opacity(0.5), radius: 10, y: 4)
                        Text(initials)
                            .font(.system(size: 24, weight: .black))
                            .foregroundColor(Color(hex: "#BF360C")!)
                    }
                    ...
                    VStack(alignment: .leading, spacing: 5) {
                        ...
                        Text(displayEmail)
                            ...
                            .foregroundColor(Color(hex: "#FFCCBC")!)
                            .lineLimit(1)
                    }
                    .padding(.top, 6)
    ...
    private var aboutGroup: some View {
        SettingsGroup(header: "ABOUT", headerIcon: "info.circle.fill") {
            ...
            Button {
                if let url = URL(string: "https://apps.apple.com/app/homvi") {
                    UIApplication.shared.open(url)
                }
            } label: {
                HStack(spacing: 14) {
