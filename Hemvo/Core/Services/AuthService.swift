...
final class AuthService {
    ...
    func verifySubscription(transactionID: UInt64) async -> SubscriptionVerification {
        ...
        do {
            ...
            // A 200 response is Apple's definitive verdict: `active:false` means
            // the transaction is genuinely expired/revoked/unknown.
            return response.active ? .active : .invalid
        } catch {
            // Network blip, edge-function error, or a non-2xx (e.g. 409 "bound to
            // another account"): we could NOT determine the truth. Report it as
    ...
    /// Result of asking the verify-subscription Edge Function about a StoreKit
    /// transaction. Distinguishes Apple's definitive "not valid" verdict from a
    /// transient failure so a network/server error never expires a paying owner.
    enum SubscriptionVerification {
        case active        // Apple confirmed a real, unexpired, unrevoked transaction
        case invalid       // Apple confirmed the transaction is not valid
        case unverifiable  // could not complete the check — status unknown, leave row as-is
    }
