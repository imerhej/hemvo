...
// ⚠️  LEGACY — This service validates invite codes locally against UserDefaults,
// which is insecure (invite state lives only on the inviter's device and can be
// tampered on a jailbroken phone). All invite logic has been migrated to
// HouseholdService + the `send-invite-email` Supabase Edge Function, which
// validates tokens server-side. This class is kept only to avoid breaking
// any remaining build references but should not be used for new features.
final class HouseholdInviteService {
    ...
    /// - Warning: Local-only validation. Use HouseholdService.joinHousehold(token:) instead.
    @available(*, deprecated, renamed: "HouseholdService.joinHousehold")
    ...
    func sendInviteEmail(invite: HouseholdInvite, senderName: String) {
        let subject = "\(senderName) invited you to join their Hemvo household"

        let body = """
Hi \(invite.name),

\(senderName) has invited you to join their household on Hemvo — the all-in-one home management app.
...
Steps to join:
1. Download Hemvo from the App Store:
   https://apps.apple.com/app/hemvo
