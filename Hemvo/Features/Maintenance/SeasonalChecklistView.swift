...
// MARK: - SeasonalItem model (used by SeasonalViewModel / MaintenanceView badge)
struct SeasonalItem: Codable, Identifiable {
    var id:         UUID   = UUID()
    var title:      String
    var note:       String = ""
    var isComplete: Bool   = false
    var season:     String          // Season.rawValue key
}

// MARK: - SeasonalViewModel (used by MaintenanceView badge)
@MainActor
final class SeasonalViewModel: ObservableObject {
    ...
    func items(for season: Season) -> [SeasonalItem] {
        items.filter { $0.season == season.rawValue }
    }

    func add(title: String, note: String, season: Season) {
        guard !title.trimmingCharacters(in: .whitespaces).isEmpty else { return }
        items.append(SeasonalItem(
    ...
    func toggle(_ item: SeasonalItem) {
        if let idx = items.firstIndex(where: { $0.id == item.id }) {
            items[idx].isComplete.toggle(); persist()
        }
    }

    func delete(_ item: SeasonalItem) {
        items.removeAll { $0.id == item.id }; persist()
    }

    func resetSeason(_ season: Season) {
        for idx in items.indices where items[idx].season == season.rawValue {
            items[idx].isComplete = false
        }
        persist()
    }

    func clearSeason(_ season: Season) {
        items.removeAll { $0.season == season.rawValue }; persist()
    }
    ...
    private func load() {
        guard let data = UserDefaults.standard.data(forKey: key),
              let decoded = try? JSONDecoder().decode([SeasonalItem].self, from: data)
        else { return }
        items = decoded
    }
}
