...
//       homebase://reset-password?token=XXXXXX
//  4. User taps the link on their iPhone → iOS calls scene(_:openURLContexts:)
//  5. HomeBaseApp receives the URL via .onOpenURL and calls handleResetURL(_:)
...
final class PasswordResetService {
    ...
    /// Returns the token string from homebase://reset-password?token=XXXXX
    func tokenFromURL(_ url: URL) -> String? {
        guard url.scheme == "homebase",
