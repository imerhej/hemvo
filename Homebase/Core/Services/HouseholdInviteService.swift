...
final class HouseholdInviteService {
    ...
    func sendInviteEmail(invite: HouseholdInvite, senderName: String) {
        let subject = "\(senderName) invited you to join their HomeBase household"

        let body = """
Hi \(invite.name),

\(senderName) has invited you to join their household on HomeBase — the all-in-one home management app.
...
Steps to join:
1. Download HomeBase from the App Store:
   https://apps.apple.com/app/homebase
